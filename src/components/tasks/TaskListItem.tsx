...
export function TaskListItem({
  ...
  const [progress, setProgress] = useState<number>(task.completed ? 100 : 0);
  const [inputVal, setInputVal] = useState<string>(task.completed ? "100" : "0");

  // Sync progress when task.completed changes externally
  useEffect(() => {
    const next = task.completed ? 100 : 0;
    setProgress(next);
